...
BasicComponent({
  externalClasses: ['search-button-class', 'input-class', 'search-inner-class'],
  properties: {
    // 当前输入的值
    value: String,
  ...
  observers: {
    center(newVal) {
      this.setData({ showVirtualPlaceholder: newVal });
    }
  },
  methods: {
    onClickVirtualInput() {
      const data = { showVirtualPlaceholder: false, focus: true, cursor: 0 };
      if (this.data.value !== '') {
    ...
    onBlur() {
      const data = { showSearch: false, showVirtualPlaceholder: false };
      if (this.data.center) {
