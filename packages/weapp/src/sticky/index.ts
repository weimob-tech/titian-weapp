...
BasicComponent<
  ...
  WechatMiniprogram.Component.MethodOption,
  {
    ...
    containerObserver: WechatMiniprogram.IntersectionObserver;
  }
>({
  properties: {
    offsetTop: {
  ...
  methods: {
    disabledChanged(newVal: boolean) {
    ...
    initObserver() {
      this.destroyObserver();
      getRect(this, STICKY_TARGET).then((rect: { height: number }) => {
        this.setData({
          height: rect.height
        });
    ...
    createObserver() {
      ...
      contentObserver.observe(STICKY_TARGET, (res) => {
        this.setData({ width: res.boundingClientRect.width });
    ...
    creatContainerObserver() {
      ...
      this.getContainerRect().then((rect: { height: number; top: number }) => {
        getRect(this, STICKY_TARGET).then((contentRect: { height: number; top: number }) => {
    ...
    setFixed(top: number, containerObserverTop?: number) {
      let fixed = false;
      if (this.data.usePureCss) {
