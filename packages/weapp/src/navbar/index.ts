...
BasicComponent({
  externalClasses: ['ext-title-class', 'ext-subtitle-class'],
  properties: {
    type: {
      type: String,
  ...
  data: {
    ...
    navbarHeight: 0,
    gap: 0,
    prefixIcons: [] as unknown[],
    backIconName: 'nav-back',
    homeIconName: 'home',
    menuClientRect: { top: 0, height: 0, width: 0 },
  ...
  observers: {
    ...
    'subtitle, subtitleHeight': function fn() {
      ...
      this.setNavbarStyle(menuClientRect as WechatMiniprogram.ClientRect, statusBarHeight);
    },
  ...
  lifetimes: {
    attached() {
      ...
      this.setData({ statusBarHeight, menuClientRect, gap });
  ...
  methods: {
    setNavbarStyle(menuClientRect: WechatMiniprogram.ClientRect, statusBarHeight: number) {
      const gap = menuClientRect.top - statusBarHeight;
      ...
      this.setData({ navbarHeight, navbarPlaceholderHeight });
    },
    setPrefixIcons() {
      ...
      this.setData({ prefixIcons });
    },
    setOpacity(e: { scrollTop: number }) {
      const { scrollTop } = e;
    ...
    updateFixedTop({ selector, height, isTitian = true, cursorSpacing = 0 }: IDetailData) {
      this.getDiff({ selector, height, isTitian, cursorSpacing }).then((diff) => {
        if (height) {
          if (diff > 0) {
            this.setData({ top: diff });
    ...
    getScrollTop({ selector, height, isTitian = true, cursorSpacing = 0, currentScrollTop }: IDetailData) {
      return new Promise((resolve) => {
        this.getDiff({ selector, height, isTitian, cursorSpacing }).then((diff: number) => {
          if (height) {
            if (diff <= 0) {
              this._currentScrollTop = undefined;
            } else {
              this._currentScrollTop = currentScrollTop || 0;
              resolve({ placeholderHeight: diff, scrollTop: this._currentScrollTop + diff });
            }
          } else if (this._currentScrollTop !== undefined) {
            resolve({ placeholderHeight: 0, scrollTop: this._currentScrollTop });
          }
        });
      });
    },
