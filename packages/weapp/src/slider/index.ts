...
BasicComponent({
  externalClasses: ['track-class', 'rail-class', 'thumb-class'],
  properties: {
    max: {
      type: Number,
      value: 100
    ...
    'thumbPercent1, thumbPercent2': function thumbPercentChanged(thumbPercent1, thumbPercent2) {
      ...
      if (Array.isArray(value) && (_thumbValue1 !== _originThumbValue1 || _thumbValue2 !== _originThumbValue2)) {
        ...
        this.triggerEvent('change', {
          value: [_thumbValue1, _thumbValue2]
        });
      } else if (_thumbValue1 !== _originThumbValue1) {
        this.triggerEvent('change', { value: _thumbValue1 });
      }
    }
  },
  methods: {
    init() {
      ...
      wx.createSelectorQuery()
        .in(this)
        .select('.titian-slider-rail')
        .fields({ id: true, dataset: true, rect: true, size: true })
        .exec((rect = []) => {
          this.setData({
            _railSize: [rect[0].width, rect[0].height],
            _railPosition: [rect[0].left, rect[0].top]
          });
        });
    },
    _stepChanged(step: number, oldStep?: number) {
      if (step === oldStep) {
        return;
      }
    ...
    _valueChanged(value: number | number[], oldValue?: number | number[]) {
      ...
      if (isRangeValue) {
        ...
        [thumbPercent1, thumbPercent2] = [_originThumbValue1, _originThumbValue2].map((v) =>
          valueToPercent(v, min, max)
        );
      } else {
    ...
    onThumbTouchMove(event: WechatMiniprogram.TouchEvent) {
