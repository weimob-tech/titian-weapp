...
import uploaderBehavior, { UploaderData, UploaderThis } from '../behaviors/uploader';
import BasicComponent from '../common/basic/BasicComponent';
import { UploadActionType, UploadFile, UploadStatus, UploadType } from '../common/interface/uploader';
...
>({
  ...
  properties: {
    ...
    value: {
      ...
      observer(newVal) {
        ...
        this.updateData(newVal as UploadFile[]);
      }
    },
    immediately: { type: Boolean, value: true },
  ...
  methods: {
    ...
    onSelect(event: WechatMiniprogram.TouchEvent) {
      ...
      this.choose(this.data.fileKeyList, this.fileMap)
        .then((res: UploadFile[]) => {
          if (!res) {
            return;
          }
          if (this.controlled) {
            this.triggerEvent('change', {
              file: null,
              uploading: false,
              fileList: this.data.fileKeyList
                .map((fileName) => {
          ...
          this.setData(
            {
              fileKeyList: [...fileKeyList, ...res.map((item) => item.key)],
              fileMap: this.fileMap
            },
            () => {
              // 状态翻转
        ...
        .catch((err: WechatMiniprogram.GeneralCallbackResult) => {
          this.triggerEvent('error', { status: 'other', message: err.errMsg });
        });
    },
    onPreviewImage(event: WechatMiniprogram.TouchEvent) {
      ...
      wx.previewImage({
        urls: fileKeyList
          .filter((item) => fileMap[item].fileType === UploadType.IMAGE && fileMap[item].status !== UploadStatus.FAIL)
          .map((item) => fileMap[item].path),
        current: path,
        fail: () => {
    ...
    onPreviewVideo(event: WechatMiniprogram.TouchEvent) {
      const { key } = event.currentTarget.dataset;
      const { fileKeyList } = this.data;
      const { fileMap } = this;
      const video = fileKeyList.filter(
        (item) => fileMap[item].fileType === UploadType.VIDEO && fileMap[item].status !== UploadStatus.FAIL
      );
