Component({
  ...
  properties: {
    ...
    calloutStyle: {
      ...
      observer(newVal) {
        ...
        this.setData({
          cssVariableObj: { ...cssVariableObj, ...newCssVar }
        });
      }
    },
    markers: {
  ...
  observers: {
    markers: function markersChanged(markers: []) {
      const innerMarkers = markers.map((m: any) => ({
        ...m,
  ...
  methods: {
    setDefaultHeight(newVal?: number) {
      if (!newVal) {
        ...
        query.exec((res) => {
          ...
          this.setData({
            cssVariableObj: { ...this.data.cssVariableObj, ...newCssVar }
          });
        });
      } else {
        const newCssVar = {
    ...
    clickMarker(event: any) {
      ...
      this.data.mapContext.moveToLocation({ latitude, longitude });
      // console.log('clickMarker - this.data.hasAlwaysDisplayCallout', this.data.hasAlwaysDisplayCallout);
      if (this.data.hasAlwaysDisplayCallout) {
        this.switchAlwaysDisplayCallout({ latitude, longitude });
      }
    },

    clickCallout(event: any) {
      const { markerId } = event;
      const marker = this.data.markers.find((i) => i.id === markerId);
    ...
    findNearestAndSetCenter() {
      ...
      wx.getLocation({
        ...
        success: (res) => {
          ...
          let minDistance = -1;
          let nearestMarker = { latitude: 0, longitude: 0 };
          innerMarkers.forEach((m) => {
            const p2 = { latitude: m.latitude, longitude: m.longitude };
    ...
    includePointsAndMoveToCenter(center: { longitude: number; latitude: number }) {
    ...
    switchAlwaysDisplayCallout(position?: { longitude: number; latitude: number }) {
      const { innerMarkers } = this.data;
