...
>({
  externalClasses: ['ext-class', 'tree-select-sidebar', 'tree-select-container'],
  properties: {
    extStyle: String,
    options: {
      type: Array,
  ...
  observers: {
    'options, alias': function fn(value, alias) {
      if (value.length > 0) {
        ...
        this.setData({ list: this.data.options[index][childrenAlias] || [] });
      }
    }
  },
  methods: {
    onClick(event: WechatMiniprogram.CustomEvent) {
      ...
      this.triggerEvent('change-nav', { index, item });
    },
    onSelect(event: WechatMiniprogram.CustomEvent) {
      const { index } = event.currentTarget.dataset;
      const valueAlias = (this.data.alias.value || 'value') as keyof ListColumn;
      ...
      this.triggerEvent('change-item', { activeValue, current, item });
    }
  }
});
