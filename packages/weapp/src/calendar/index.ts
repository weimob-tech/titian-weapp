...
BasicComponent<
  ...
  {
    contentObserver: WechatMiniprogram.IntersectionObserver;
    monthTitle: string;
    calendar: Calendar;
    status: string;
  }
>({
  externalClasses: ['ext-popup-class', 'ext-popup-mask-class', 'ext-popup-content-class', 'ext-scroll-class'],
  properties: {
    start: {
      type: String,
      value: WEEK[0].name
    ...
    color: { type: String, value: '#2580FF' },
    round: Boolean,
    title: { type: String, value: '日期选择' },
    formatter: null,
    confirmText: {
      type: String,
      value: '确定'
  ...
  observers: {
    ...
    'minDate,maxDate,mode,maxRange,allowSameDay,start,value,defaultValue,maxSize,formatter': function (
      minDate,
      maxDate,
      mode,
      maxRange,
  ...
  methods: {
    onSelect(e: WechatMiniprogram.TouchEvent) {
      ...
      if (updateData) {
        ...
        this.triggerEvent('select', { date, currentDate: this.calendar.data });
      }
    },
    ...
    updateDataByProps({
      ...
      this.setData({ monthList: this.calendar.monthList, week: this.calendar.week }, () => {
        this.triggerEvent('select', {
          date: null,
          currentDate: this.calendar.data
        });
        if (this.data.visible && !this.calendar.controlled) {
          this.setData({ id: this.calendar.id });
        }
      });
    },
    onClose() {
