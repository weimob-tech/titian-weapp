...
export type Option = { [key: string | number]: unknown };

export type Fn = (date?: unknown) => Option[] | Promise<Option[]>;
type IteratorBack = { done: boolean; value: Option[] };
export function isValidOption(options: unknown): options is Option[] {
...
export default class Cascade {
  ...
  getResult({ columnValueList, code }: { columnValueList: Option[]; code: string }) {
    const { cascade } = this;
