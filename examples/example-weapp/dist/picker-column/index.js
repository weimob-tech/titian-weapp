 ...
 */ BasicComponent({
    ...
    methods: {
        ...
        calcDistance ({ columns , value  }) {
            ...
            if (rowAlias === null) {
                ...
                row = columns.findIndex((item)=>item[rowAlias] === value && !isDisabled(item));
            }
            if (row === -1) {
            ...
            if (rowAlias === null) {
                ...
                resetValue = columns[row][rowAlias];
            }
     ...
     * picker-column 组合使用
     * 在wxs中调用
     */ synchronous ({ isTop , isBottom , rowIndex , colAlias  }) {
            if (!this.parent) {
                if (isTop) {
