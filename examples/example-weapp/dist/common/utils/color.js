/* eslint-disable no-nested-ternary */ /* eslint-disable prefer-regex-literals */ /* eslint-disable no-bitwise */ const HSBToRGB = (h, s, b)=>{
    s /= 100;
    b /= 100;
    const k = (n)=>(n + h / 60) % 6;
...
const hexToRGB = (hex, a)=>{
    ...
    a = a || `${h & 0x000000ff}`;
    return `rgb${isAlpha ? 'a' : ''}(${h >>> (alpha ? 24 : 16)}, ${(h & (alpha ? 0x00ff0000 : 0x00ff00)) >>> (alpha ? 16 : 8)}, ${(h & (alpha ? 0x0000ff00 : 0x0000ff)) >>> (alpha ? 8 : 0)}${isAlpha ? `, ${a}` : ''})`;
};
...
const getNeutralsGrey = (precent = 85, diffs)=>{
    const group = [
        '#f2f2f2',
        '#f5f5f5',
        '#fafafa',
