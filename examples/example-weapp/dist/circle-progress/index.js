...
BasicComponent({
    ...
    lifetimes: {
        attached () {
            ...
            getFieldStyle(this, selector, [
                'color'
            ]).then((res)=>{
                const color = RGBAToHex(res.color);
                this.setData({
                    color: color || this.data.color || '#fa2c19'
                });
                wx.createSelectorQuery().in(this).select(selector).fields({
                    node: true,
                    size: true
    ...
    methods: {
        ...
        handleDraw (curValue, targetValue, zIndex, color, defaultEndAngle) {
            const { ctx  } = this;
        ...
        drawBuffer () {
            ...
            this.handleDraw(this.currentBufferValue, this.targetBufferValue, 2, bufferBgColor || hexToRGB(this.data.color, 0.3));
        },
        drawBg () {
