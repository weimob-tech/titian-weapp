...
let RangeCalendarCalc = class RangeCalendarCalc extends CalendarCalc {
    ...
    *select({ calDateList , actionCalDate  }) {
        const nextCalDate = this.changeStatus({
            calDateList,
            actionCalDate
        });
    ...
    changeStatus({ calDateList , actionCalDate  }) {
        ...
        if (before.fullDateNum === actionCalDate.fullDateNum && this.allowSameDay) {
            return [
                {
                    ...actionCalDate,
                    status: DateStatusEnum.RANGE_FULL
                }
            ];
        }
        return [
    ...
    calcUpdateData({ prevData , nextData , monthList  }) {
        ...
        prevData.forEach((item)=>{
            ...
            if (typeof this.formatter === 'function') {
                curRangeData[key] = this.formatter({
                    ...item,
        ...
        nextData.forEach((item, index, cur)=>{
            const key = `monthList[${item.parentIndex}].dateList[${item.day - 1}]`;
            if (typeof this.formatter === 'function') {
                if (item.status === DateStatusEnum.RANGE_FULL) {
                    const start = this.formatter({
                        ...item,
                        status: DateStatusEnum.RANGE_START
                    });
                    const end = this.formatter({
                        ...item,
                        status: DateStatusEnum.RANGE_END
                    });
                    const bottomInfo = [
            ...
            curRangeData[monthCurDateKey] = cur.map((i)=>({
                    ...i
                }));
        });
            ...
            for(let i = 0; i < len; i += 1){
                ...
                if (i !== startIndex && i !== endIndex) {
                    if (i > startIndex && i < endIndex) {
                        ...
                        const monthCoverKey1 = `monthList[${i}].isCover`;
                        if (item.isCover) {
                            curRangeData[monthCoverKey1] = false;
                        }
                    }
                }
            }
        } else if (nextData.length === 1 && prevData.length === 2) {
            const len1 = monthList.length;
            for(let i1 = 0; i1 < len1; i1 += 1){
                const item1 = monthList[i1];
                if (item1.isCover) {
                    const monthCoverKey2 = `monthList[${i1}].isCover`;
                    curRangeData[monthCoverKey2] = false;
                }
            }
        }
        return curRangeData;
    }
    constructor(params, { allowSameDay , maxRange  }){
        super(params);
...
export { RangeCalendarCalc as default };
