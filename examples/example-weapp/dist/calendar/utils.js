...
export function getCalDate(date) {
...
export const getWeek = (startWeek = WEEK[0].name, week = WEEK)=>{
...
export function calcMonthData({ min , max  }, currentDate) {
    let number = (max.year - min.year) * 12 + (max.month - min.month) + 1;
    number = number > 0 ? number : 1;
    let id = '';
    const renderDate = [];
    const monthList = new Array(number).fill(0).map((item, index)=>{
 ...
 * @param mode 类型
 * @returns booolean
 */ export function judgeCover(month, select, mode) {
 ...
 * @param mode 类型
 * @returns booolean
 */ export function judgeCompare(month, select, mode) {
    if (mode !== CalTypeEnum.RANGE) {
        return false;
    }
    return select.some((i)=>i.parentIndex === month.parentIndex);
}
function getCalculateStatus(day, border, select) {
    ...
    if (border.min && border.max) {
        status = border.min.fullDateNum > day.fullDateNum || day.fullDateNum > border.max.fullDateNum ? DateStatusEnum.DISABLED : DateStatusEnum.NOT_STATUS;
    } else {
        if (border.min) {
...
const getRenderDateByCalendar = ({ calDate , select , border , index , formatter  })=>{
...
// eslint-disable-next-line @titian-design/no-more-args
export function getDayByMonth({ month , select , border , formatter , parentIndex  }) {
    ...
    return new Array(number).fill(null).map((item, index)=>{
        ...
        return getRenderDateByCalendar({
            calDate,
            select,
            border,
            index: parentIndex,
...
export const mergeContinuousDate = (list)=>{
    let temp = [
        list[0]
    ];
    const result = [
    ...
    for(let x = 0; x < result.length; x += 1){
        ...
        curList.forEach((item, index)=>{
            ...
            if (index === 0) {
                resetResult.push({
                    ...item,
                    status: DateStatusEnum.MULTIPLE_START
                });
                return;
            }
            if (index === curLength - 1) {
                resetResult.push({
                    ...item,
                    status: DateStatusEnum.MULTIPLE_END
                });
                return;
            }
            resetResult.push({
                ...item,
                status: DateStatusEnum.MULTIPLE_MIDDLE
            });
        });
    }
    return resetResult;
