...
BasicComponent({
    ...
    lifetimes: {
        ...
        attached () {
            ...
            if (visibleItemCount !== 0) {
                ...
                wx.createSelectorQuery().in(this).select('.titian-picker-content').boundingClientRect((rect)=>{
                    if (rect) {
                        if (rect.height === 0) {
                            this.height = Number.parseInt(height, 10);
                            this.top = Math.round((height * VISIBLEITEMCOUNT - height) / 2);
        ...
        ready () {
            ...
            if (this.helper.value !== value || this.options !== options || this.cascade !== cascade) {
                ...
                this.setData(this.helper.changeProps({
                    options,
                    value,
                    rowAlias,
                    useRowIndex,
    ...
    methods: {
        ...
        updateDistance (visibleItemCount) {
            const update = {
                curLoading: false,
                height: this.height,
                top: this.top
