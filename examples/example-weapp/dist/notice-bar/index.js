...
BasicComponent({
    ...
    lifetimes: {
        attached () {
            if (Array.isArray(this.data.content)) {
                this.setData({
                    text: this.data.content.join(' ')
                });
                this.setData({
                    list: this.data.content
                });
            } else {
                this.setData({
    ...
    methods: {
        setBgColor () {
            getFieldStyle(this, '.titian-notice-bar', [
                'color',
                'width',
                'height'
            ]).then((res)=>{
        ...
        initHorizontalAnimation () {
            ...
            Promise.all([
                getRect(this, '.titian-notice-bar-container'),
                getRect(this, '.titian-notice-bar-content')
            ]).then((res)=>{
                ...
                // #ifdef MP-WEIXIN
                const creatAnimation = ()=>this.animate(NOTICE_TARGET, [
                        {
        ...
        initVerticalAnimation () {
            ...
            getRect(this, '.titian-notice-bar').then((res)=>{
                const wrapHeight = res.height;
                // #ifdef MP-WEIXIN
                const duration = this.data.list.length * wrapHeight / this.data.speed * 1000;
                const keyframes = this.data.list.map((item, index)=>({
                        translateY: -index * wrapHeight,
                        ease: 'ease-in-out'
                    }));
                const creatAnimation = ()=>this.animate(NOTICE_TARGET, keyframes, duration, ()=>{
        ...
        horizontalAnimation (firstDuration, duration, leftPosition, rightPosition) {
            const animation = wx.createAnimation({});
            this.setData({
                animationData: animation.translateX(leftPosition).step({
            ...
            const circulation = (time)=>{
                const timer = setTimeout(()=>{
                    this.setData({
                        animationData: animation.translateX(rightPosition).step({
                            duration: 0
                        }).export()
                    });
                    setTimeout(()=>{
        ...
        verticalAnimation (wrapHeight) {
            const animation = wx.createAnimation({
                timingFunction: 'ease-in-out'
            });
            const firstDuration = wrapHeight / this.data.speed * 1000;
            const keyframes = this.data.list.map((item, index)=>-index * wrapHeight);
            const n = keyframes.length - 1;
            let i = 0;
            const circulation = (time, timeout)=>{
                const timer = setTimeout(()=>{
                    ...
                    this.setData({
                        animationData: animation.translateY(translateY).step({
                            duration
                        }).export()
                    });
