...
import getCurYear, { DateTimePickerColumnEnum, DateTimePickerEnum, DateTimePickerHelper, getOrderPickerValue, getOrderValue, DateTimeTypeMapping } from './utils';
...
BasicComponent({
    ...
    observers: {
        'value,minDate,maxDate,type, label, sort, formatter, filter': function(...params) {
            ...
            if (this.helper.value !== value || this.helper.minDate !== minDate || this.helper.maxDate !== maxDate || this.helper.type !== type || this.helper.label !== label || this.helper.sort !== sort || this.helper.formatter !== formatter || this.helper.filter !== filter) {
                this.updateValue({
                    value,
                    minDate,
                    maxDate,
    ...
    methods: {
        onCalendarChange (e) {
            ...
            if (type === DateTimePickerEnum.YEARMONTH && colAlias === DateTimePickerColumnEnum.YEAR || type === DateTimePickerEnum.DATE && (colAlias === DateTimePickerColumnEnum.YEAR || colAlias === DateTimePickerColumnEnum.MONTH) || type === DateTimePickerEnum.DATETIME && (colAlias === DateTimePickerColumnEnum.YEAR || colAlias === DateTimePickerColumnEnum.MONTH || colAlias === DateTimePickerColumnEnum.DAY || colAlias === DateTimePickerColumnEnum.HOUR)) {
                const newValue = getOrderPickerValue(type, value).map((item)=>{
            ...
            this.triggerEvent('change', {
                type,
                value: this.helper.standardValue
            });
        },
        onCalendarCancel (e) {
