/* eslint-disable @typescript-eslint/no-explicit-any */ /* eslint-disable @typescript-eslint/ban-ts-comment */ import BasicComponent from '../common/basic/BasicComponent';
...
var ETabsDivider;
(function(ETabsDivider) {
    ETabsDivider[/** 细线分割 */ "LINE"] = 'line';
    ETabsDivider[/** 阴影分割 */ "SHADOW"] = 'shadow';
    ETabsDivider[/** 无分割 */ "DEFAULT"] = 'default';
})(ETabsDivider || (ETabsDivider = {}));
var ETabsVariant;
(function(ETabsVariant) {
    ETabsVariant[/** 滑块风格 */ "BLOCK"] = 'block';
    ETabsVariant[/** 纯净型 */ "PURE"] = 'pure';
    ETabsVariant[/** 多行类型 */ "MULTI"] = 'multi';
    ETabsVariant[/** 日期类型 */ "DOUBLE"] = 'calendar';
})(ETabsVariant || (ETabsVariant = {}));
BasicComponent({
    externalClasses: [
        'tab-active-class',
        'swiper-class',
    ...
    observers: {
        'count, tabs, activeTab': function fn(count, tabs, activeTab) {
            ...
            if (tabs.length > 0 && isPlainObject(tabs[0])) {
                tabsName = tabs.map((el)=>el[this.data.alias.label || this.data.tabKey]);
            }
            if (tabs.length > 0 && isString(activeTab)) {
                activeIndex = tabsName.findIndex((el)=>activeTab === el);
            }
            if (this.data.focusUpdate || tabsLength.length !== tabsName.length) {
                tabsLength = tabsName.map((_, index)=>index);
            }
    ...
    methods: {
        ...
        setTabWidth (count) {
            ...
            getRect(this, TABS_TARGET).then((dom)=>{
                ...
                if (this.data.gap < 0) {
                    ...
                    nextTick(()=>this.setData({
                            currentCount
                        }));
                    return;
                }
                // 处理自定义gap, 等间距模式
                this.createSelectorQuery().selectAll(TAB_TARGET).boundingClientRect((rect)=>{
                    // @ts-ignore
                    ...
                    let once = false;
                    // @ts-ignore
                    const tabTotalWidth = rect.reduce((p, c, index)=>{
                        const translate = p + c.width / 2;
