...
export function formatDate(timeData, formatStr, tag) {
    const time = {
    ...
    if (tag === 'group') {
        ...
        keyMap.forEach((value, key)=>{
            if (formatStr.includes(key) && !res[value]) {
                res[value] = time[key];
            }
        });
        return res;
    ...
    // 返回所有数据
    return formatStr.replace(new RegExp(`${Object.keys(time).join('|')}`, 'g'), (subStr)=>time[subStr] || '');
}
export function isDifferentTime(time1, time2, formatStr) {
    const actionsMap = new Map([
        ...
        [
            's',
            ()=>Math.floor(time1 / 1000) !== Math.floor(time2 / 1000)
        ],
        [
            'm',
            ()=>Math.floor(time1 / 60000) !== Math.floor(time2 / 60000)
        ],
        [
            'H',
            ()=>Math.floor(time1 / 3600000) !== Math.floor(time2 / 3600000)
        ],
        [
            'D',
            ()=>Math.floor(time1 / 86400000) !== Math.floor(time2 / 86400000)
        ]
    ]);
